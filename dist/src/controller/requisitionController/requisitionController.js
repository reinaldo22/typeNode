...
class RequisitionController {
    requisition(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const requisitionRepository = typeorm_1.getRepository(Requisition_1.default);
            const { name, description } = req.body;
            const requisition = requisitionRepository.create({
                name,
                description
            });
            yield requisitionRepository.save(requisition);
            return res.status(200).json({ message: 'Requisição criada com sucesso!' });
        });
    }
}
exports.default = new RequisitionController();
